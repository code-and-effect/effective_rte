...
(function ($) {
    ...
    function render(element, options) {
        ...
        if (element.tag.isSelfClosing) {
            ...
            if (!element.isRoot && renderTag) {
                // close
                output.push(">");
                if(element.tag.needsExtraLine) { output.push('\n'); }
            }
            ...
            if (!element.isRoot && renderTag) {
                ...
                if (options.format && element.tag.needsExtraLine) output.push('\n');
            }
        }
    ...
    function Tag(name, close, rawAttributes, options) {
        ...
        this.needsExtraLine = $.inArray(this.name, tagNeedsExtraLine) > -1;
